...
| 301 | `model_route_policy` | mounts/59-router.js (Router v0.2: allowlisted policy metadata only — routeId, executionMode, dataClass, trainingAllowed, primaryProvider, primaryModel, fallbackCount, reasonCodes; capability and execution context are never logged) |
