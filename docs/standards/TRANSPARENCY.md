...
| 169 | `audit_search_read` | mounts/131-audit-search.js (FS-N2: {by, filters, count} — operator searched audit_chain; filter keys + count only, never raw payloads; TG_AUDIT_SEARCH=1 only) |
| 170 | `audit_search_denied` | mounts/131-audit-search.js (FS-N2: {bot} — non-operator touched /v2/audit/search; RBAC refusal audited) |
