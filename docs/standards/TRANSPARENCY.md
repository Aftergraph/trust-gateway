...
| 144 | `telemetry_tenant_read` | mounts/123-telemetry-tenant.js (FS-K2: {by, tenant, count} — operator queried /v2/tenants/:id/telemetry; payloadSummary only, never raw args/steps/text; TG_TELEMETRY_TENANT_SCOPED=1 only) |
| 145 | `telemetry_tenant_denied` | mounts/123-telemetry-tenant.js (FS-K2: {bot, tenant, reason} — non-operator touched a tenant telemetry route, or cross-tenant query refused 404 anti-enumeration; tenant id + reason only) |
