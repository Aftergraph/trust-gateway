...
const { AlertSink, getAlertSink, parseUrls, ALERT_RATE_LIMIT_MS, ALERT_SUPPRESS_MS, ALERT_SUPPRESS_MAX, ALERT_TIMEOUT_MS } =
...
function fakeWebhook() {
  ...
  const server = http.createServer((req, res) => {
    ...
    req.on('end', () => {
      ...
      deliveries.push({ path: req.url, method: req.method, auth: req.headers.authorization || null, body: parsed });
...
test('alerting: TG_ALERT_URLS parse — multiple URLs, whitespace, empty → none', () => {
  assert.deepStrictEqual(parseUrls('https://a.example/hook,https://b.example/hook'),
    ['https://a.example/hook', 'https://b.example/hook']);
  assert.deepStrictEqual(parseUrls('  https://a.example  ,  ,https://b.example ,'),
...
test('alerting: TG_ALERT_TOKEN → Bearer header on each delivery', async () => {
  const f = recordingFetch();
  const sink = new AlertSink({ urls: ['https://a.example/hook'], token: 'sekrit-tok', fetchImpl: f });
  await sink.alert('watchdog_fail', {});
  assert.strictEqual(f.calls[0].opts.headers.authorization, 'Bearer sekrit-tok');
});

test('alerting: field projection — objects/arrays never reach the payload', async () => {
  const f = recordingFetch();
  const sink = new AlertSink({ urls: ['https://a.example/hook'], fetchImpl: f });
  await sink.alert('watchdog_fail', {
    count: 3,
    ok: true,
    note: null,
...
test('alerting: rate limit — max 1 per type per 60s (2nd within window dropped)', async () => {
  const f = recordingFetch();
  let t = 0;
  const sink = new AlertSink({ urls: ['https://a.example/hook'], fetchImpl: f, now: () => t });
...
test('alerting: timeout constant is 3000ms and passed to fetch as AbortSignal.timeout', async () => {
  ...
  const f = recordingFetch();
  const sink = new AlertSink({ urls: ['https://hang.example/hook'], fetchImpl: f });
  await sink.alert('watchdog_fail', {});
  const sig = f.calls[0].opts.signal;
  assert.ok(sig instanceof AbortSignal, 'alert() must pass a signal (3s timeout)');
});
...
test('alerting: getAlertSink(gw) reads TG_ALERT_URLS env when constructed', () => {
  ...
  try {
    ...
    sink.configure({ fetchImpl: recordingFetch(), urls: null, env: process.env });
    assert.ok(sink);
  } finally {
    if (prev === undefined) delete process.env.TG_ALERT_URLS; else process.env.TG_ALERT_URLS = prev;
  }
});
...
test('alerting: watchdog exit codes unchanged — 0 when all green (alert env unset)', async () => {
  ...
  try {
    const out = await sh([REPO_ROOT], { PORT: String(srv.port), TG_DATA_DIR: dataDir });
...
test('alerting: watchdog exit 1 on gateway down even with TG_ALERT_URLS set', async () => {
  const dataDir = fs.mkdtempSync(path.join(os.tmpdir(), 'tg-wd-data-'));
  const out = await sh([REPO_ROOT], { PORT: '59901', TG_DATA_DIR: dataDir, TG_ALERT_URLS: 'https://alerts.example/hook' });
...
test('alerting: watchdog POSTs alert JSON to local collector on failure, exit stays 1', async () => {
  ...
  try {
    const dataDir = fs.mkdtempSync(path.join(os.tmpdir(), 'tg-wd-data-'));
    const out = await sh([REPO_ROOT], { PORT: '59902', TG_DATA_DIR: dataDir, TG_ALERT_URLS: hook.url });
    assert.strictEqual(out.code, 1);
    assert.ok(out.stdout.includes('healthz unreachable'));
    assert.strictEqual(hook.deliveries.length, 1, 'deliveries=' + JSON.stringify(hook.deliveries));
...
test('alerting: watchdog alert POST failure must not change exit code', async () => {
  const dataDir = fs.mkdtempSync(path.join(os.tmpdir(), 'tg-wd-data-'));
  const out = await sh([REPO_ROOT], { PORT: '59903', TG_DATA_DIR: dataDir, TG_ALERT_URLS: 'https://127.0.0.1:1/hook' });
  assert.strictEqual(out.code, 1);
});
...
function spawnGateway() {
  return new Promise((resolve) => {
    ...
    const child = require('node:child_process').spawn('node', ['-e', `
...
test('alerting: restore_refused → alert fired alongside audit entry (operator signal)', async () => {
  ...
  sink.configure({
    ...
    fetchImpl: async (url, opts) => { delivered.push({ url, opts }); return { ok: true }; },
  });
  const srv = await serve(gw);
  try {
    const res = await fetch(srv.port, 'POST', '/v2/backup/restore', OPERATOR, { name: 'backup-2099-01-01T00-00-00Z' });
...
function sh(cwd, env) {
  ...
  return new Promise((resolve) => {
    require('node:child_process').execFile('bash', ['deploy/watchdog.sh'], {
      cwd: cwd[0],
      ...
      const code = err ? (err.code === undefined || err.code === null ? 1 : err.code) : 0;
...
function tmpFile() {
  return path.join(fs.mkdtempSync(path.join(os.tmpdir(), 'tg-alert-')), 'telemetry.json');
}
...
function fetch(port, method, p, token, body) {
  return new Promise((resolve, reject) => {
    const req = http.request({
      ...
      res.on('end', () => {
        let json = null;
        try { json = JSON.parse(s); } catch { /* non-JSON */ }
