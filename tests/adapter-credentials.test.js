...
test('stores an adapter secret in Vault without returning its value', () => {
  ...
  const result = lifecycle.setSecret({ tenant: 'tenant-a', adapterId: 'adp_0001', secretName: 'Token', value: 'runtime-secret-1' });
  assert.deepEqual(result, { ok: true, tenant: 'tenant-a', adapterId: 'adp_0001', secretName: 'token' });
  assert.deepEqual(v.calls, [['setSecret', 'tenant-a', 'adapters/adp_0001/credentials/token']]);
...
test('enforces tenant and adapter binding before inspect, revoke, or resolve', () => {
  const h = handles();
  const { lifecycle } = makeLifecycle(undefined, h);
  assert.throws(() => lifecycle.inspectHandle({ handleId: 'ch_adapter_test', tenant: 'tenant-b', adapterId: 'adp_0001' }), { code: 'adapter_credential_scope_mismatch' });
  assert.throws(() => lifecycle.revokeHandle({ handleId: 'ch_adapter_test', tenant: 'tenant-a', adapterId: 'adp_0002' }), { code: 'adapter_credential_scope_mismatch' });
  assert.throws(() => lifecycle.resolveForBroker({ handleId: 'ch_adapter_test', tenant: 'tenant-b', adapterId: 'adp_0001', request: {} }), { code: 'adapter_credential_scope_mismatch' });
...
test('delegates revoke and broker resolution only after binding checks', () => {
  const h = handles();
  const { lifecycle } = makeLifecycle(undefined, h);
  const revoked = lifecycle.revokeHandle({ handleId: 'ch_adapter_test', tenant: 'tenant-a', adapterId: 'adp_0001', reason: 'operator_rotation' });
  assert.equal(revoked.revokedReason, 'operator_rotation');
  const resolved = lifecycle.resolveForBroker({ handleId: 'ch_adapter_test', tenant: 'tenant-a', adapterId: 'adp_0001', request: { tenant: 'tenant-a' } });
