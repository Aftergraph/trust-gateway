...
// db.js. The bloat-guard test appends 12500+ entries which holds the SQLite
// connection open long enough for parallel tests to collide on the default
// data/gateway.db.
const TG_J3_DB_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "tg-j3-db-"));
process.env.TG_DB_FILE = path.join(TG_J3_DB_DIR, "gateway.db");
...
function makeChain(dir, old, fresh) {
  ...
  for (let i = 0; i < old; i++)
    chain.append({ type: "old", i }, oldBase + i * 1000);
  const nowTs = Date.now();
  for (let i = 0; i < fresh; i++)
    chain.append({ type: "fresh", i }, nowTs + i * 1000);
  return chain;
}
...
test("restoreArchive: duplicate-skip idempotency — second restore restores nothing, chain unchanged", () => {
  return withEnv((dir) => {
    ...
    assert.equal(
      ...
      60,
      "every archived entry was a duplicate",
    );
...
test("restoreArchive: checksum mismatch REFUSES (throws) and leaves the live chain untouched", () => {
  return withEnv((dir) => {
    ...
    assert.throws(
      ...
      (e) =>
        e.code === "checksum_mismatch" && /sha256 mismatch/.test(e.message),
    );
  ...
  await withEnv(async (dir) => {
    ...
    try {
      const r = await fetch(
        s.port,
        "POST",
        "/v2/chain/archive/2026-09-03/restore",
        WK,
        "{}",
      );
  ...
  await withEnv(async (dir) => {
    ...
    try {
      const r = await fetch(
        s.port,
        "POST",
        "/v2/chain/archive/1999-01-01/restore",
        OP,
        "{}",
      );
